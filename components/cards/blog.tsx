    ...
    <div className={cn(" grid gap-4 group", blog.isVertical ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2')}>
      <Link href={`/blogs/${blog.slug}`}>
        <div className="relative bg-secondary rounded-md">
          <Image width={650} height={335} src={blog.image.url} alt={blog.title} className="px-2 md:px-7 rounded-md group-hover:-translate-y-7 -translate-y-6 transition-all object-cover grayscale group-hover:grayscale-0 max-md:-translate-y-2 max-md:group-hover:-translate-y-3" />
        </div>
      </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <Link href={`/tags/${blog?.tag?.slug}`}>
              <Badge variant={'secondary'} role="button" ><Tag className="w-3 h-3 me-2"/>{blog.tag?.name}</Badge>
            </Link>
            <Link href={`/categories/${blog?.category?.slug}`}>
              <Badge variant={'secondary'} role="button" >
              <Layers2 className="w-3 h-3 me-2"/>
                {blog.category?.name}</Badge>
            </Link>
          </div>
